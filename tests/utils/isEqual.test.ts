...
describe("isEqual", () => {
	...
	test("should return true for object that has circular dependency", () => {
		type Circular = { foo: string; self?: Circular };
		const a: Circular = { foo: "bar" };
		const b: Circular = { foo: "bar" };
	...
	test("should return false for different object that has circular dependency", () => {
		type Circular = { baz: string; self?: Circular };

		const aChild: Circular = { baz: "qux" };
		const bChild: Circular = { baz: "quux" };
